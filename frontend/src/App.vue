...
async function submitModal(){loading.value=true;error.value='';try{
  ...
  closeModal();await loadAll();notify('信息已保存')
...
<template>
  ...
  </div><div class="modal-foot"><button type="button" class="btn btn-ghost" @click="closeModal">取消</button><button class="btn btn-primary" :disabled="loading">{{loading?'保存中…':'保存'}}</button></div></form></div>
